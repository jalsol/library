struct Centroid {
    ...
    Centroid() : n(0) {}
    Centroid(int _n) : n(_n + 5) {
        sz.resize(_n + 5);
        min.resize(_n + 5);
        processed.resize(_n + 5);
        g.resize(_n + 5);
    }
    ...
    void getAns(int u, int p, int64_t sum, bool mark, int depth = 1) {
        if (depth > k) return;
        maxdepth = std::max(maxdepth, depth);

        if (mark) {
            min[depth] = std::min(min[depth], sum);
        }
        else {
            ans = std::min(ans, sum + min[k - depth]);
        }
